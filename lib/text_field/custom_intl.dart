...
class CustomIntl extends StatefulWidget {
  ...
  @override
  State<CustomIntl> createState() => _CustomIntlState();
}

class _CustomIntlState extends State<CustomIntl> {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      style: widget.style,
      onSaved: (value) {
        // Strip formatting chars (e.g. dashes) before handing to caller
      ...
      onChanged: (value) async {
        // Strip formatting chars before handing to caller
        final digits = value.replaceAll(RegExp(r'[^\d]'), '');
      ...
      validator: (value) {
        // Strip formatting chars (dashes, spaces) — only count raw digits
        final digits = (value ?? '').replaceAll(RegExp(r'[^\d]'), '');
      ...
      // maxLength is null when inputFormatters handle the cap — dashes would
      // otherwise count as characters and reduce how many digits can be typed.
      maxLength: (widget.inputFormatters?.isNotEmpty == true || widget.disableLengthCheck)
          ? null
