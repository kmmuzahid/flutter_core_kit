...
abstract class CoreKitStorage {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      // Test write to verify secure storage works on this device
      await _secureStorage!.write(key: '_core_kit_test', value: 'ok');
      await _secureStorage!.delete(key: '_core_kit_test');
      _useSecure = true;
    } catch (_) {
      // Secure storage failed (rooted device, old Android, etc.)
      // Fall back silently — never fail
      _useSecure = false;
      try {
        _fallbackStorage = await SharedPreferences.getInstance();
      } catch (_) {
        // SharedPreferences failed too (unlikely, but guard)
      }
    }
    _initialized = true;
  }
  ...
  static Future<void> write(String key, String value) async {
    ...
    try {
      ...
      // If even the primary method fails mid-operation, try fallback
      try {
        _fallbackStorage ??= await SharedPreferences.getInstance();
        await _fallbackStorage!.setString(key, value);
        _useSecure = false;
      } catch (_) {
        // Absolute last resort — in-memory only for this session
        // Token will be lost on app restart but won't crash
      }
    }
  }
